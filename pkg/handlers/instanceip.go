...
func (r *InstanceIP) GetReferences(obj interface{}) []contrail.ResourceReference {
	...
	res, ok := obj.(*contrail.InstanceIP)
	if ok {
...
func (r *InstanceIP) Search(name, namespace, kind string, path []string) map[string]*InstanceIP {
	var resMap = make(map[string]*InstanceIP)
	...
	for idx := range nodeList {
		n := r.dbClient.Get("InstanceIP", nodeList[idx].Name, nodeList[idx].Namespace)
		if r, ok := n.(*contrail.InstanceIP); ok {
			resMap[r.Namespace+"/"+r.Name] = &InstanceIP{InstanceIP: r}
		}
	}
	return resMap
}
