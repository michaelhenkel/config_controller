...
func (r *RoutingInstance) GetReferences(obj interface{}) []contrail.ResourceReference {
	...
	res, ok := obj.(*contrail.RoutingInstance)
	if ok {
		objRef := contrail.ResourceReference{
			ObjectReference: res.Spec.Parent,
		}
		resourceReferenceList = append(resourceReferenceList, objRef)
	}
...
func (r *RoutingInstance) FindFromNode(node string) []pbv1.Response {
	...
	virtualMachineList := virtualMachine.Search(node, "", "VirtualRouter", []string{"RoutingInstance", "VirtualNetwork", "VirtualMachine", "VirtualMachineInterface"})
...
func (r *RoutingInstance) Search(name, namespace, kind string, path []string) map[string]*RoutingInstance {
	var resMap = make(map[string]*RoutingInstance)
	...
	for idx := range nodeList {
		n := r.dbClient.Get("RoutingInstance", nodeList[idx].Name, nodeList[idx].Namespace)
		if r, ok := n.(*contrail.RoutingInstance); ok {
			resMap[r.Namespace+"/"+r.Name] = &RoutingInstance{RoutingInstance: r}
		}
	}
	return resMap
}
